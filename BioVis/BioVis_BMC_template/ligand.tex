...
\begin{figure}[tb]
  ...
  Haloalkane dehalogenase (blue) with the original trajectories of three ligands, TCP--294, TCP--295, and TCP--296 (grey, orange, and red).
  }
\end{figure}  

The hypothesis stated by the biochemists at the beginning was that all three ligands should bind to the same pocket surrounded by the following amino acids {\color{red}TODO add IDs}.
...
First, our tool allows to select the reference{\color{red}/observed?} ligand and thus easily compare their individual trajectories.
...
\begin{figure}[tb]
  ...
  \subref{fig:tcp3_dist_time_average_294}~TCP--294: caption
  \subref{fig:tcp3_dist_time_average_295}~TCP--295: caption, better resolution
  \subref{fig:tcp3_dist_time_average_296}~TCP--296: caption
  }
\end{figure} 
...
Additionally, the user can select just a small portion of the simulation in order to evaluate the ligand transportation through the protein molecule (see Figure~\ref{fig:path}).
...
\todo[inline]{replace straightness for stuckness}
