...
The hypothesis stated by the biochemists at the beginning was that all three ligands should bind to the same pocket surrounded by the following amino acids {\color{red} H--269, L--243, L--206}.
...
First, the user can select a desired ligand to be observed and thus easily compare their individual trajectories.
...
TCP--295 reached the deeply buried active site first and hence filled the empty space around this active site.
...
\includegraphics[width=0.95\linewidth]{img/tcp3_3D.pdf}
  \caption{
  \label{fig:tcp3_3D}
	3D view showing the mutual position of two ligand TCP-294 and TCP-296 inside the binding site.
  }
\end{figure} 
...
Nevertheless, some additional observations revealing more details about the simulation can be done afterwards as well.  
Among others, there is obviously a spike in the distance of TCP--296 from the active site around time step 30.000. 
This corresponds to the time when the TCP--294 started to head towards the active site (see Figure~\ref{}).

Another example is using the scatterplot showing the relationship between the distance from the active site and the stuckness of the ligand (see Figure~\ref{fig:tcp3_dist_stuckness_295}).
From this scatterplot we can see that the ligand was highly oscillating in a close vicinity of the active site.
This can suggest that the active site is very stable with respect to its ability to bind a ligand.
...
\begin{figure}[tb]
  ...
  Scatterplot showing the relationship between the distance from the active site and the stuckness of ligand TCP-296.
  }
\end{figure}

Finally, the user can select just a small portion of the simulation in order to evaluate the ligand transportation through the protein molecule (see Figure~\ref{fig:path}).
