...
TODO!!!!! After the first evaluation of the overall ligand trajectory the biochemists stated that they would like to evaluate the interactions between the ligand and protein in more detail in two particular scenarios (see Figure~\ref{fig:case_overview}). 
First, they focused on the part where the ligand traveled from the active site to the protein surface (see Section~\ref{sec:case1}). 
...
The exploration of these two parts of the simulation allows the biochemists to closely evaluate the influence of the protein tunnel and its properties to the ligand movements and it may help to reveal the essence of the interaction between the observed protein and given ligand.

For the first case the biochemists selected the first part of the simulation consisting from circa $17.000$ time steps (see Figure~\ref{fig:case_overview}). 
...
For instance, by using the scatterplot set to show the ligand distance from the active site over time (see Figure~\ref{fig:case_example1} -- top) we immediately revealed that the ligand was actually near to the protein surface (represented by high peaks) in few additional times during this part of the simulation.
...
\begin{figure}[htb]
  ...
  \caption{\label{fig:case_hydro_distance} {Top -- the distance from the active site over time. As can be seen the most hydrophilic amino acids are surrounding the most distant parts of the trajectory. Bottom -- hydrophobicity profile of the trajectory.}}
\end{figure} 
