...
Here the biochemists selected the TODO green amino acids (asparagine with ID 38 and aspartic acid with ID 103). 
...
The hypothesis stated by the biochemists at the beginning was that all three ligands should bind to the same pocket surrounded by the following amino acids H--269, L--243, L--206.
...
First, the user can select one of the ligands to be observed and thus easily compare their individual trajectories.
%In the first stage of analysis biochemists again used overview of the overall simulation behavior.
Using the simulation overview (see Figure~\ref{fig:tcp3_overview}) it is obvious that only TCP--296 evidently entered the protein -- orange and red colors depict the state when the ligand is inside of protein while blue color indicates the ligand closeness to the protein surface. 
...
In case of the TCP--294, the overview is mostly blue (indicating closeness to the surface) due to the fact that the site where this ligand binds is relatively shallow and close to the surface.
TCP--295 reached the deeply buried active site first and hence filled the empty space around  {\color{red}this active site} \todo{I would suggest to replace this by "it"}.
...
\includegraphics[width=0.95\linewidth]{img/tcp3_3D.pdf}
  \caption{
  \label{fig:tcp3_3D}
	3D view showing the mutual position of two ligand TCP-294 and TCP-296 inside the binding site at the end of simulation.
  }
\end{figure} 
...
When exploring molecular simulations containing a ligand, one of the most important tasks is to determine ability of protein active site to bind the given ligand.
In this particular case the biochemist were able to easily estimate ability of the active site to bind TCP--296 and TCP--294 directly from distance scatter plots (see Figure~\ref{fig:tcp3_distances}).
It can be clearly seen that both ligands were presented in vicinity of the active site for long time and they both never left the binding site after accessing it.  
Nevertheless, in more complicated cases the biochemist can use scatter plot showing the relationship between the distance from the active site and the stuckness of the ligand.
For instance, in case of TCP--296 (see Figure~\ref{fig:tcp3_dist_stuckness_295}) it is easy to confirm that the ligand was highly oscillating (was stuck) in a close vicinity of the active site.
On the other hand, it can be also seen that with the growing distance from active site the stuckenss is getting lower.
...
\begin{figure}[tb]
  ...
  Scatterplot showing the relationship between the distance from the active site and the stuckness of ligand TCP-296. {\color{red} what about merging Fig15 and this one?}
  }
\end{figure}

Finally, as can be seen from distance scatter plot (see Figure~\ref{fig:tcp3_distances}) both TCP--294 and TCP--296 entered the protein active site during short period of time and never left it afterwards.
At this point the biochemist wanted to explore this particular moments in more details.
Our tool is highly suitable for such use case since all views are interactively lined together.
Therefore, the biochemist can, for instance, easily select the given part of simulation in scatter plot and immediately observe the ligand transportation through the protein molecule in 3D (see Figure~\ref{fig:path}).
