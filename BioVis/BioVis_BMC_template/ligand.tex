...
\label{sec:case1}
For the first case the biochemist selected the first part of the simulation consisting from circa 17000 time steps. 
From this point the biochemist can continue in more detailed exploration using bruising and linking technique inside other three views -- namely inside the 3D, scatter plot and bar chart view (see Figure~\ref{fig:case_example1}).
...
For instance, using the scatter plot set to show distance from active site over time immediately revealed that the the ligand was actually near the protein surface (represented by high peaks) few additional times during this part of the simulation. \todo{ref for image, maybe}


The same behavior can be actually seen also from the simulation overview {\color{red} (see Figure~\ref{fig:case_overview} -- look for the slim orange lines in first big green block from left)} but its is much more obvious in the scatter plot view.
Moreover, the scatter plot view enables to select these time steps (or the opposite) in much more convenient way. 

Another interesting behavior can be seen with the same setup (see Figure~\ref{fig:case_example1}) is that the movement of the ligand from active site (lover values) to the surface (high values) and back always happens in relatively short period of time.   


\begin{figure}[htb]
	\centering
  \includegraphics[width=0.95\linewidth]{img/case_example_1.png}
  \caption{\label{fig:case_example1} {\color{red}TODO}}
\end{figure}
