...
Such representation of $S$ was chosen in order to enable simplification of different parts of $T_{in}$ using different magnitude.
...
Then, it is decided whether $T'$ can be incrementally updated to obtain $T_{out}$. \textcolor{red}{How is the decision made? is the following correct?}
...
The algorithm starts with considering the whole trajectory as complex, i.e., set of complex intervals $C$ is set to the interval spanning $T_{in}$.
...
We evaluate the complexity of $T$ in a neighborhood of $x$ instead of in a single point in order to take into account more global shape of the trajectory.
Our typical setting for $\nu$ is 2 \angstrom\ and it was obtained experimentally.
...
First, a set of simple point $P$ is found by thresholding $c(x)$ by $\tau$.
